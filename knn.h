...
public:
    ...
    KNN(Point_Set& _ps)
        : ps(_ps)
        , image_size(_ps.image_size)
        , n_labels(_ps.n_labels)
    {
    }
 ...
 *
 * NOTE: Using transform_reduce instead of transform + accumulate makes my code 10x faster!
 */
    double distance(image_it pt_beg, image_it pt_end, image_it q_beg)
    {
        return std::transform_reduce(pt_beg, pt_end, q_beg, 0,
                     std::plus<>(),          // reduce fcn
                     [](const auto a, const auto b) { return (int(a) - int(b)) * (int(a) - int(b)); }) / 16384.0; // transform
    }
